...
/** Services */
import { abbreviate, comma, formatBytes, isMobile, sortArrayOfObjects, mon, truncateDecimalPart } from "@/services/utils"
...
const buildChart = (chart, data) => {
	...
	const y = d3.scaleLinear([0, maxValue], [height - margin.axisX, margin.top])
	...
	svg.append("g")
		...
		.each(function (d) {
			...
			if (!isMobile()) {
				text.attr("transform", "rotate(-45)").attr("x", -10).attr("y", 10).style("text-anchor", "end")
			} else {
				text.attr("transform", "rotate(-90)").attr("x", -10).attr("y", -5).style("text-anchor", "end")
			}
		})
				...
				<div v-if="tooltip.show" :class="$style.tooltip_wrapper">
					...
					>
						...
						<div
							...
							:style="{
								background: `linear-gradient(90deg, rgba(10, 222, 113, 0%) 0%, ${tooltip.data.color}, rgba(10, 222, 113, 0%) 100%)`,
							}"
						/>
					</Flex>
				</div>
